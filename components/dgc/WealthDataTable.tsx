...
function CellDetail({ cell, label }: { cell: StatCell; label: string }) {
  if (cell.value === null) {
    return (
      <div className="rounded-lg border border-white/10 bg-white/5 p-3 text-sm">
        ...
        <p className="mt-1 text-white/50">N/A</p>
        {cell.methodologyNote ? (
          <p className="mt-2 text-xs text-white/45">{cell.methodologyNote}</p>
        ) : null}
      </div>
    );
  }
  ...
  return (
    <div className="rounded-lg border border-white/10 bg-white/5 p-3 text-sm">
      ...
      {cell.definition ? (
        <p className="mt-2 text-xs text-white/55">{cell.definition}</p>
      ) : null}
      {cell.methodologyNote ? (
        <p className="mt-1 text-xs text-white/45">{cell.methodologyNote}</p>
      ) : null}
      {cell.source ? (
              ...
              return (
                <Fragment key={row.reportYear}>
                  ...
                  >
                    ...
                    <td className="px-3 py-2.5 text-white/70">{row.tier}</td>
                    {COLUMN_KEYS.map((key) => (
                      <td key={key} className="px-3 py-2.5 text-white/85">
